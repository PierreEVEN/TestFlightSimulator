...
Shader::Shader(const std::filesystem::path& source_mesh_path, EShaderKind in_shader_kind) : shader_kind(in_shader_kind)
{
    auto shader_data = read_shader_file(source_mesh_path);
    if (shader_data)
    {
        if (auto bytecode = compile_module(source_mesh_path.string(), shader_data.value(), shader_kind))
        {
...
std::optional<std::vector<uint32_t>> Shader::compile_module(const std::string& file_name, const std::string& shader_code, EShaderKind shader_kind, bool optimize)
{
    ...
    switch (shader_kind)
    {
    case EShaderKind::VertexShader:
        shader_stage = GLSLANG_STAGE_VERTEX;
        break;
    case EShaderKind::FragmentShader:
        shader_stage = GLSLANG_STAGE_FRAGMENT;
        break;
    case EShaderKind::GeometryShader:
