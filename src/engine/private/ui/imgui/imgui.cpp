...
/*
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE
     ...
     while (true)
     {
        ...
        io.DisplaySize.x = 1920.0f;             // set the val display width
        io.DisplaySize.y = 1280.0f;             // set the val display height here
 ...
 - 2019/12/17 (1.75) - [undid this change in 1.76] made Columns() limited to 64 columns by asserting above that limit. While the val code technically supports it, future code may not so we're putting the restriction ahead.
...
// - We don't do 'val += 2; continue;' after handling ### to keep the code smaller/faster (measured ~10% diff in Debug build)
// FIXME-OPT: Replace with e.g. FNV1a hash? CRC32 pretty much randomly access 1KB. Need to do proper measurements.
...
{
    IM_ASSERT(GImGui != NULL && "No val context. Did you call ImGui::CreateContext() and ImGui::SetCurrentContext() ?");
    return GImGui->Style;
}
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id)
{
    ...
    if (g.ActiveId == id && g.FocusTabPressed && !IsActiveIdUsingKey(ImGuiKey_Tab) && g.FocusRequestNextWindow == NULL)
    {
        ...
        g.FocusRequestNextCounterTabStop = window->DC.FocusCounterTabStop + (g.IO.KeyShift ? (is_tab_stop ? -1 : 0) : +1); // Modulo on index will be applied at the end of frame once we've got the total id of items.
    }
...
{
    IM_ASSERT(GImGui != NULL && "No val context. Did you call ImGui::CreateContext() or ImGui::SetCurrentContext()?");
...
void ImGui::UpdateTabFocus()
{
    ...
    // Turn queued focus request into val one
...
void ImGui::NewFrame()
{
    ...
    // Setup val font and draw list shared data
...
void ImGui::Render()
{
    ...
    for (int n = 0; n < g.Viewports.Size; n++)
    {
        ...
        // (note we scale cursor by val viewport/monitor, however Windows 10 for its own hardware cursor seems to be using a different scale factor)
    ...
    // (Legacy) Call the Render callback function. The val prefer way is to let the user retrieve GetDrawData() and call the render function themselves.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
// NB- Rectangle is clipped by our val clip setting
...
static ImVec2 CalcWindowSizeAfterConstraint(ImGuiWindow* window, ImVec2 new_size)
{
    ...
    {
        // Using -1,-1 on either X/Y axis to preserve the val size.
...
static bool ImGui::UpdateWindowManualResize(ImGuiWindow* window, const ImVec2& size_auto_fit, int* border_held, int resize_grip_count, ImU32 resize_grip_col[4], const ImRect& visibility_rect)
{
    ...
    //   This is however not the case with val back-ends under Win32, but a custom borderless window implementation would benefit from it.
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Late create viewport if we don't fit within our val host viewport.
        ...
        if (window->ViewportOwned)
        {
            ...
            // Either a SetNextWindowPos() call in the val frame or a SetWindowPos() call in the previous frame may have this effect.
        ...
        if (!window->Collapsed)
        {
            // When reading the val size we need to read it after size constraints have been applied.
...
bool ImGui::ItemAdd(const ImRect& bb, ImGuiID id, const ImRect* nav_bb_arg)
{
    ...
    // We need to calculate this now to take account of the val clipping rectangle (as items like Selectable may change them)
    if (IsMouseHoveringRect(bb.Min, bb.Max))
...
void ImGui::EndGroup()
{
    ...
    // If the val ActiveId was declared within the boundary of our group, we copy it to LastItemId so IsItemActive(), IsItemDeactivated() etc. will be functional on the entire group.
...
void ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags, ImGuiTooltipFlags tooltip_flags)
{
    ...
    {
        // The default tooltip position is a little offset to give space to see the context menu (it's also clamped within the val viewport/monitor)
...
bool ImGui::IsPopupOpen(ImGuiID id, ImGuiPopupFlags popup_flags)
{
    ...
    {
        // Return true if any popup is open at the val BeginPopup() level of the popup stack
    ...
    else
    {
        ...
        {
            // Return true if the popup is open at the val BeginPopup() level of the popup stack (this is the most-common query)
...
// Popup identifiers are relative to the val ID-stack (so OpenPopup and BeginPopup needs to be at the same level).
...
//  information are available, it may represent the entire platform monitor from the frame of reference of the val viewport.
...
static bool ImGui::NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    // We perform scoring on items bounding box clipped by the val clipping rectangle on the other axis (clipping on our movement axis would give us equal scores for all clipped items)
    ...
    if (quadrant == g.NavMoveDir)
    {
        // Does it beat the val best candidate?
        ...
        if (dist_box == result->DistBox)
        {
            ...
            else if (dist_center == result->DistCenter)
            {
                ...
                // (with higher index) to the right/downwards by an infinitesimal amount since we the val "best" button already (so it must have a lower index),
...
static void ImGui::NavProcessItem(ImGuiWindow* window, const ImRect& nav_bb, const ImGuiID id)
{
    ...
    // FIXME-NAV: Consider policy for double scoring (scoring from NavScoringRectScreen + scoring from a rect wrapped according to val wrapping policy)
...
static void ImGui::NavUpdate()
{
    ...
    // If we initiate a movement request and have no val NavId, we initiate a InitDefautRequest that will be used as a fallback if the direction fails to find a match
    ...
    // For scoring we use a single segment on the left side our val item bounding box (not touching the edge to avoid box overlap with zero-spaced items)
    ...
    if (page_up_held != page_down_held || home_pressed != end_pressed) // If either (not both) are pressed
    {
        ...
        else
        {
            ...
            else if (home_pressed)
            {
                ...
                // Preserve val horizontal position if we have any.
...
bool ImGui::BeginDragDropSource(ImGuiDragDropFlags flags)
{
    ...
    if (!(flags & ImGuiDragDropFlags_SourceExtern))
    {
        ...
        if (source_id == 0)
        {
            ...
            // We build a throwaway ID based on val ID stack + relative AABB of items in window.
...
// We split text into individual lines to add val tree level padding
void ImGui::LogRenderedText(const ImVec2* ref_pos, const char* text, const char* text_end)
{
    ...
    for (;;)
    {
        // Split the string. Each new line (after a '\n') is followed by spacing corresponding to the val depth of our log entry.
...
void ImGui::LoadIniSettingsFromMemory(const char* ini_data, size_t ini_size)
{
    ...
    for (char* line = buf; line < buf_end; line = line_end + 1)
    {
        ...
        {
            // Parse "[Type][Name]". Note that 'Name' can itself contains [] characters, which is acceptable with the val format and parsing code.
            line_end[-1] = 0;
...
static void ImGui::UpdateViewportsNewFrame()
{
    ...
    // Create/update main viewport with val platform position.
...
static void ImGui::UpdateSelectWindowViewport(ImGuiWindow* window)
{
    ...
    // Mark window as allowed to protrude outside of its viewport and into the val monitor
    if (!lock_viewport)
    {
        if (flags & (ImGuiWindowFlags_Tooltip | ImGuiWindowFlags_Popup))
        {
...
// (Different from DockSettingsHandler_ClearAll() + DockSettingsHandler_ApplyAll() because this reuses val settings!)
...
void ImGui::DockBuilderRemoveNodeChildNodes(ImGuiID root_id)
{
    ...
    // Make sure we don't lose our val pos/size. (FIXME-DOCK: Consider tidying up that code in DockNodeAddWindow instead)
    if (root_node)
    {
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    if (ImGui::TreeNode("Internal state"))
    {
        ...
        ImGui::Text("HoveredId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d", g.HoveredId, g.HoveredIdPreviousFrame, g.HoveredIdTimer, g.HoveredIdAllowOverlap); // Data is "in-flight" so depending on when the Metrics window is called we may see val frame information or not
