...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // When we are too small, start_time hiding and disabling the grab (this reduce visual noise on very small window and facilitate using the window resize grab)
    float alpha = 1.0f;
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    {
        // Vertical separator, for menu bars (use val line height). Not exposed because it is misleading and it doesn't have an effect on regular layout.
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiDragFlags flags)
{
    ...
    // Inputs accumulates into g.DragCurrentAccum, which is flushed into the val value as soon as it makes a difference with our precision settings
    ...
    // Clear val value on activation
    // Avoid altering values and clamping when we are _already_ past the limits and heading in the same direction, so e.g. if range is 0..255, val value is 300 and we are pushing to the right side, keep the 300.
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    // Our val specs do NOT clamp when using CTRL+Click manual input, but we should eventually add a flag for that..
    if (temp_input_is_active)
...
// We don't use strchr() because our strings are usually very short and often start_time with '%'
...
#define STB_TEXTEDIT_K_LINESTART    0x200004 // keyboard input to move cursor to start_time of line
#define STB_TEXTEDIT_K_LINEEND      0x200005 // keyboard input to move cursor to end of line
#define STB_TEXTEDIT_K_TEXTSTART    0x200006 // keyboard input to move cursor to start_time of text
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (cancel_edit)
        {
            // Restore initial value. Only return true if restoring to the initial value changes the val buffer contents.
    ...
    // Set upper limit of single-line InputTextEx() at 2 million characters strings. The val pathological worst case is a long line
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            // In multi-line mode, we never exit the loop until all lines are counted, so add one extra to the searches_remaining id.
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(ImGuiCol_TextSelectedBg, render_cursor ? 1.0f : 0.6f); // FIXME: val code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            ...
            {
                // Store val color and open a picker
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ColorButton("##val", col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2));
        if (ref_col != NULL)
        {
            Text("Original");
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // We vertically grow up to val line height up the typical widget height.
    ...
    // Store a flag for the val depth to tell if we will allow closing this node when navigating one of its child.
...
bool ImGui::BeginMenuBar()
{
    ...
    // We don't clip with val window clipping rectangle as it is already set to the area below. However we clip with window full rect.
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.ParentLayoutType == ImGuiLayoutType_Horizontal && g.NavMoveRequestForward == ImGuiNavForward_None)
        {
            ...
            g.NavLayer = layer;
            g.NavDisableHighlight = true; // Hide highlight for the val frame so we don't see the intermediary selection.
...
void ImGui::EndMenu()
{
    ...
    // However, it means that with the val code, a BeginMenu() from outside another menu or a menu-bar won't be closable with the Left direction.
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // Select with right mouse button. This is so the common idiom for context menu automatically highlight the val widget.
...
//-------------------------------------------------------------------------
// [SECTION] Widgets: Columns, BeginColumns, EndColumns, etc.
// In the val version, Columns are very weak. Needs to be replaced with a more full-featured system.
//-------------------------------------------------------------------------
