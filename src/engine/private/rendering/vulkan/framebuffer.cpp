...
void Framebuffer::create_framebuffer_images()
{
	...
	vulkan_texture::create_image_2d(GfxContext::get(), buffer_size.width, buffer_size.height, 1, static_cast<VkSampleCountFlagBits>(window->get_msaa_sample_count()), colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, color_target, color_memory);
        vulkan_texture::create_image_view_2d(GfxContext::get(), color_target, color_view, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);

	/** Depth buffer */
        VkFormat depthFormat = vulkan_utils::get_depth_format(GfxContext::get()->physical_device);
        vulkan_texture::create_image_2d(GfxContext::get(), buffer_size.width, buffer_size.height, 1, static_cast<VkSampleCountFlagBits>(window->get_msaa_sample_count()), depthFormat, VK_IMAGE_TILING_OPTIMAL,
                                        VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depth_target, depth_memory);
        vulkan_texture::create_image_view_2d(GfxContext::get(), depth_target, depth_view, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);
	...
	swapchain_image_views.resize(window->get_image_count());
        vkGetSwapchainImagesKHR(GfxContext::get()->logical_device, swapchain->get(), &swapchain_image_count, nullptr);
        vkGetSwapchainImagesKHR(GfxContext::get()->logical_device, swapchain->get(), &swapchain_image_count, swapchain_images.data());
	for (size_t i = 0; i < swapchain_images.size(); i++)
	{
            vulkan_texture::create_image_view_2d(GfxContext::get(), swapchain_images[i], swapchain_image_views[i], window->get_surface_format().format, VK_IMAGE_ASPECT_COLOR_BIT, 1);
	}
}
