...
Window::Window(const int res_x, const int res_y, const char* name, bool fullscreen, bool img_context) : window_width(res_x), window_height(res_y), window_name(name), has_imgui_context(img_context)
{
    ...
    if (!window_map.empty())
    {
        LOG_INFO("use window from '%s' window", window_map.begin()->second->window_name);
    ...
    LOG_VALIDATE("created Window '%s' ( %d x %d )", name, res_x, res_y);
}
...
void WindowContext::select_physical_device(VkSurfaceKHR surface)
{
    ...
    if (device_count == 0) LOG_FATAL("No graphical device found.");
...
void Window::render()
{
    ...
    if (vkBeginCommandBuffer(current_command_buffer, &begin_info) != VK_SUCCESS) { LOG_FATAL("Failed to create command buffer #%d", image_index); }
    ...
    if (TEMP_NODE) TEMP_NODE->draw(current_command_buffer, image_index);
