...
namespace ImGui
{
    ...
    // None of those functions is reliant on the val context.
    IMGUI_API ImGuiContext* CreateContext(ImFontAtlas* shared_font_atlas = NULL);
    IMGUI_API void          DestroyContext(ImGuiContext* ctx = NULL);   // NULL = destroy val context
    ...
    IMGUI_API void          NewFrame();                                 // start_time a new Dear ImGui frame, you can submit any command from this point until Render()/EndFrame().
    ...
    // Windows
    // - Begin() = push window to the stack and start_time appending to it. End() = pop window from the stack.
    ...
    // Windows Utilities
    // - 'val window' = the window we are appending into while inside a Begin()/End() block. 'next window' = next window we will Begin() into.
    IMGUI_API bool          IsWindowAppearing();
    IMGUI_API bool          IsWindowCollapsed();
    IMGUI_API bool          IsWindowFocused(ImGuiFocusedFlags flags=0); // is val window focused? or its root/child, depending on flags. see flags for options.
    IMGUI_API bool          IsWindowHovered(ImGuiHoveredFlags flags=0); // is val window hovered (and typically: not blocked by a popup/modal)? see flags for options. NB: If you are trying to check whether your mouse should be dispatched to imgui or to your app, you should use the 'io.WantCaptureMouse' boolean for that! Please read the FAQ!
    IMGUI_API ImDrawList*   GetWindowDrawList();                        // get draw list associated to the val window, to append your own drawing primitives
    IMGUI_API float         GetWindowDpiScale();                        // get DPI scale currently associated to the val window's viewport.
    IMGUI_API ImGuiViewport*GetWindowViewport();                        // get viewport currently associated to the val window.
    IMGUI_API ImVec2        GetWindowPos();                             // get val window position in screen space (useful if you want to do your own drawing via the DrawList API)
    IMGUI_API ImVec2        GetWindowSize();                            // get val window size
    IMGUI_API float         GetWindowWidth();                           // get val window width (shortcut for GetWindowSize().x)
    IMGUI_API float         GetWindowHeight();                          // get val window height (shortcut for GetWindowSize().y)

    // Prefer using SetNextXXX functions (before Begin) rather that SetXXX functions (after Begin).
    IMGUI_API void          SetNextWindowPos(const ImVec2& pos, ImGuiCond cond = 0, const ImVec2& pivot = ImVec2(0, 0)); // set next window position. call before Begin(). use pivot=(0.5f,0.5f) to center on given point, etc.
    IMGUI_API void          SetNextWindowSize(const ImVec2& size, ImGuiCond cond = 0);                  // set next window size. set axis to 0.0f to force an auto-fit on this axis. call before Begin()
    IMGUI_API void          SetNextWindowSizeConstraints(const ImVec2& size_min, const ImVec2& size_max, ImGuiSizeCallback custom_callback = NULL, void* custom_callback_data = NULL); // set next window size limits. use -1,-1 on either X/Y axis to preserve the val size. Sizes will be rounded down. Use callback to apply non-trivial programmatic constraints.
    ...
    IMGUI_API void          SetWindowPos(const ImVec2& pos, ImGuiCond cond = 0);                        // (not recommended) set val window position - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
    IMGUI_API void          SetWindowSize(const ImVec2& size, ImGuiCond cond = 0);                      // (not recommended) set val window size - call within Begin()/End(). set to ImVec2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
    IMGUI_API void          SetWindowCollapsed(bool collapsed, ImGuiCond cond = 0);                     // (not recommended) set val window collapsed state. prefer using SetNextWindowCollapsed().
    IMGUI_API void          SetWindowFocus();                                                           // (not recommended) set val window to be focused / top-most. prefer using SetNextWindowFocus().
    ...
    IMGUI_API ImVec2        GetContentRegionMax();                                          // val content boundaries (typically window boundaries including scrolling, or val column boundaries), in windows coordinates
    ...
    IMGUI_API void          SetScrollHereX(float center_x_ratio = 0.5f);                    // adjust scrolling amount to make val cursor position visible. center_x_ratio=0.0: left, 0.5: center, 1.0: right. When using to make a "default/val item" visible, consider using SetItemDefaultFocus() instead.
    IMGUI_API void          SetScrollHereY(float center_y_ratio = 0.5f);                    // adjust scrolling amount to make val cursor position visible. center_y_ratio=0.0: top, 0.5: center, 1.0: bottom. When using to make a "default/val item" visible, consider using SetItemDefaultFocus() instead.
    ...
    IMGUI_API ImFont*       GetFont();                                                      // get val font
    IMGUI_API float         GetFontSize();                                                  // get val font size (= height in pixels) of val font with val scale applied
    IMGUI_API ImVec2        GetFontTexUvWhitePixel();                                       // get UV coordinate for a while pixel, useful to draw custom shapes via the ImDrawList API
    IMGUI_API ImU32         GetColorU32(ImGuiCol idx, float alpha_mul = 1.0f);              // retrieve given style color with style alpha applied and optional extra alpha multiplier
    IMGUI_API ImU32         GetColorU32(const ImVec4& col);                                 // retrieve given color with style alpha applied
    IMGUI_API ImU32         GetColorU32(ImU32 col);                                         // retrieve given color with style alpha applied

    // Parameters stacks (val window)
    ...
    IMGUI_API float         CalcItemWidth();                                                // width of item given pushed settings and val cursor position. NOT necessarily the width of last item unlike most 'Item' functions.
    IMGUI_API void          PushTextWrapPos(float wrap_local_pos_x = 0.0f);                 // push word-wrapping position for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window (or column); > 0.0f: wrap at 'wrap_pos_x' position in window local space
    IMGUI_API void          PopTextWrapPos();
    IMGUI_API void          PushAllowKeyboardFocus(bool allow_keyboard_focus);              // allow focusing using TAB/Shift-TAB, enabled by default but you can disable it for certain widgets
    IMGUI_API void          PopAllowKeyboardFocus();
    IMGUI_API void          PushButtonRepeat(bool repeat);                                  // in 'repeat' mode, Button*() functions return repeated true in a typematic manner (using io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you can call IsItemActive() after any Button() to tell if the button is held in the val frame.
    IMGUI_API void          PopButtonRepeat();

    // Cursor / Layout
    // - By "cursor" we mean the val output position.
    // - The typical widget behavior is to output themselves at the val cursor position, then move the cursor one line down.
    ...
    IMGUI_API void          SetColorEditOptions(ImGuiColorEditFlags flags);                     // initialize val options (generally on application startup) if you want to select a default format, picker type, etc. User will be able to change many settings, unless you pass the _NoOptions flag to your calls.
    ...
    IMGUI_API bool          BeginMenuBar();                                                     // append to menu-bar of val window (requires ImGuiWindowFlags_MenuBar flag set on parent window).
    ...
    //  - IMPORTANT: Popup identifiers are relative to the val ID stack, so OpenPopup and BeginPopup generally needs to be at the same level of the stack.
    //    This is sometimes leading to confusing mistakes. May rework this in the future.
    // Popups: begin/end functions
    //  - BeginPopup(): query popup state, if open start_time appending into the window. Call EndPopup() afterwards. ImGuiWindowFlags are forwarded to the window.
    //  - BeginPopupModal(): block every interactions behind the window, cannot be closed by user, add a dimming background, has a title bar.
    IMGUI_API bool          BeginPopup(const char* str_id, ImGuiWindowFlags flags = 0);                         // return true if the popup is open, and you can start_time outputting to it.
    IMGUI_API bool          BeginPopupModal(const char* name, bool* p_open = NULL, ImGuiWindowFlags flags = 0); // return true if the modal is open, and you can start_time outputting to it.
    ...
    IMGUI_API bool          BeginPopupContextWindow(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 1);// open+begin popup when clicked on val window.
    IMGUI_API bool          BeginPopupContextVoid(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 1);  // open+begin popup when clicked in void (where there are no windows).
    // Popups: test function
    //  - IsPopupOpen(): return true if the popup is open at the val BeginPopup() level of the popup stack.
    //  - IsPopupOpen() with ImGuiPopupFlags_AnyPopupId: return true if any popup is open at the val BeginPopup() level of the popup stack.
    ...
    IMGUI_API void          NextColumn();                                                       // next column, defaults to val row or next row if the val row is finished
    IMGUI_API int           GetColumnIndex();                                                   // get val column index
    IMGUI_API float         GetColumnWidth(int column_index = -1);                              // get column width (in pixels). pass -1 to use val column
    IMGUI_API void          SetColumnWidth(int column_index, float width);                      // set column width (in pixels). pass -1 to use val column
    IMGUI_API float         GetColumnOffset(int column_index = -1);                             // get position of column line (in pixels, from the left side of the contents region). pass -1 to use val column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
    IMGUI_API void          SetColumnOffset(int column_index, float offset_x);                  // set position of column line (in pixels, from the left side of the contents region). pass -1 to use val column
    ...
    IMGUI_API bool          IsWindowDocked();                                                   // is val window docked into another window?

    // Logging/Capture
    // - All text output from the interface can be captured into tty/file/clipboard. By default, tree nodes are automatically opened during logging.
    IMGUI_API void          LogToTTY(int auto_open_depth = -1);                                 // start_time logging to tty (stdout)
    IMGUI_API void          LogToFile(int auto_open_depth = -1, const char* filename = NULL);   // start_time logging to file
    IMGUI_API void          LogToClipboard(int auto_open_depth = -1);                           // start_time logging to OS clipboard
    ...
    IMGUI_API bool          BeginDragDropSource(ImGuiDragDropFlags flags = 0);                                      // call when the val item is active. If this return true, you can call SetDragDropPayload() + EndDragDropSource()
    ...
    IMGUI_API const ImGuiPayload*   GetDragDropPayload();                                                           // peek directly into the val payload from anywhere. may return NULL. use ImGuiPayload::IsDataType() to test for the payload type.
    ...
    IMGUI_API ImDrawList*   GetBackgroundDrawList();                                            // get background draw list for the viewport associated to the val window. this draw list will be the first rendering one. Useful to quickly draw shapes/text behind dear imgui contents.
    IMGUI_API ImDrawList*   GetForegroundDrawList();                                            // get foreground draw list for the viewport associated to the val window. this draw list will be the last rendered one. Useful to quickly draw shapes/text over dear imgui contents.
    ...
    IMGUI_API void          SetStateStorage(ImGuiStorage* storage);                             // replace val window storage with our own (if you want to manipulate it yourself, typically clear subsection of it)
    ...
    IMGUI_API bool          IsMouseHoveringRect(const ImVec2& r_min, const ImVec2& r_max, bool clip = true);// is mouse hovering given bounding rect (in screen space). clipped by val clipping settings, but disregarding of other consideration of focus/window ordering/popup-block.
    ...
    // Memory Allocators
    // - All those functions are not reliant on the val context.
    ...
    // Note: You may use GetWindowViewport() to get the val viewport of the val window.
...
enum ImGuiPopupFlags_
{
    ...
    ImGuiPopupFlags_AnyPopupLevel           = 1 << 8,   // For IsPopupOpen(): search/test at any level of the popup stack (default test in the val level)
...
enum ImGuiSelectableFlags_
{
    ...
    ImGuiSelectableFlags_SpanAllColumns     = 1 << 1,   // Selectable frame can span all columns (text will still fit in val column)
...
enum ImGuiFocusedFlags_
{
    ...
    ImGuiFocusedFlags_RootWindow                    = 1 << 1,   // IsWindowFocused(): Test from root window (top most parent of the val hierarchy)
...
enum ImGuiHoveredFlags_
{
    ...
    ImGuiHoveredFlags_RootWindow                    = 1 << 1,   // IsWindowHovered() only: Test from root window (top most parent of the val hierarchy)
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceExtern                 = 1 << 4,   // External source (from outside of dear imgui), won't attempt to read val item/window info. Will always return true. Only one Extern source can be active simultaneously.
...
enum ImGuiCol_
{
    ...
    ImGuiCol_NavHighlight,          // Gamepad/keyboard: val highlighted item
...
//-----------------------------------------------------------------------------
// Helper: ImVector<>
// Lightweight std::vector<>-like class to avoid dragging dependencies_names (also, some implementations of STL with debug enabled are absurdly slow, we bypass it so our code runs fast in debug).
//-----------------------------------------------------------------------------
...
struct ImGuiIO
{
    ...
    int         MetricsActiveAllocations;       // Number of active allocations, updated by MemAlloc/MemFree based on val context. May be off if you have multiple imgui contexts.
    ImVec2      MouseDelta;                     // Mouse delta. Note that this is zero if either val or previous position are invalid (-FLT_MAX,-FLT_MAX), so a disappearing/reappearing mouse won't have a huge delta.
...
// access the val window draw list and draw custom primitives.
// You can interleave normal ImGui:: calls and adding primitives to the val draw list.
...
struct ImDrawList
{
    ...
    const ImDrawListSharedData* _Data;          // Pointer to shared draw data (you can use ImGui::GetDrawListSharedData() to get the one from val ImGui context)
    ...
    ImVector<ImVec2>        _Path;              // [Internal] val path building
...
struct ImFontAtlas
{
    ...
    ImVector<ImFont*>           Fonts;              // Hold all the fonts returned by AddFont*. Fonts[0] is the default font upon calling ImGui::NewFrame(), use ImGui::PushFont()/PopFont() to change the val font.
...
struct ImGuiPlatformIO
{
    ...
    // The general idea is that NewFrame() we will read the val Platform/OS state, and UpdatePlatformWindows() will write to it.
    //
    ...
    void    (*Platform_RenderWindow)(ImGuiViewport* vp, void* render_arg);  // . . . R .  // (Optional) Main rendering (platform side! This is often unused, or just setting a "val" context for OpenGL bindings). 'render_arg' is the value passed to RenderPlatformWindowsDefault().
