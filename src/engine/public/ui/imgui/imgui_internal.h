...
struct ImGuiPopupData;              // Storage for val popup stack
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_SelectableDontClosePopup = 1 << 5,  // false    // MenuItem/Selectable() automatically closes val Popup window
...
enum ImGuiItemStatusFlags_
{
    ...
    ImGuiItemStatusFlags_Edited             = 1 << 2,   // Value exposed by item was edited in the val frame (should match the bool return value of most widgets)
...
enum ImGuiSeparatorFlags_
{
    ...
    ImGuiSeparatorFlags_Horizontal          = 1 << 0,   // Axis default to val layout type, so generally Horizontal unless e.g. in a menu bar
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_AllowCurrentNavId     = 1 << 4,   // Allow scoring and considering the val NavId as a move target candidate. This is used when the move source is offset (e.g. pressing PageDown actually needs to send a Up move request, if we are pressing PageDown from the bottom-most item we need to stay in place)
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    bool                    CursorFollow;           // set when we want scrolling to follow the val cursor position (not always!)
...
struct ImGuiNavMoveResult
{
    ...
    float           DistBox;            // Best candidate box distance to val NavId
    float           DistCenter;         // Best candidate center distance to val NavId
    float           DistAxial;
...
{
    float               OffsetNorm;         // Column start_time offset, normalized 0.0 (far left) -> 1.0 (far right)
...
struct ImGuiContext
{
    ...
    float                   FontSize;                           // (Shortcut) == FontBaseSize * g.CurrentWindow->FontWindowScale == window->FontSize(). Text height for val window.
    ...
    int                     ViewportFrontMostStampCount;        // Every time the front-most window changes, we stamp its viewport with an incrementing id
    ...
    ImRect                  DragDropTargetRect;                 // Store rectangle of val target candidate (we favor small targets when overlapping)
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec1                  Indent;                 // Indentation / start_time position from left of window (increased by TreePush/TreePop, etc.)
    ImVec1                  ColumnsOffset;          // Offset to the val column (if ColumnsCurrent > 0). FIXME: This and the above should be a stack to allow use cases like Tree->Column->Tree. Need revamp columns API.
    ImVec1                  GroupOffset;
    ...
    int                     NavLayerActiveMaskNext; // Which layer have been written to (buffer for val frame)
    ...
    int                     FocusCounterRegular;    // (Legacy Focus/Tabbing system) Sequential id, start_time at -1 and increase as assigned via FocusableItemRegister() (FIXME-NAV: Needs redesign)
    int                     FocusCounterTabStop;    // (Legacy Focus/Tabbing system) Same, but only count widgets which you can Tab through.

    // Local parameters stacks
    // We store the val settings outside of the vectors to increase memory locality (reduce cache misses). The vectors are rarely modified. Also it allows us to not heap allocate for short-lived windows which are not using those settings.
...
struct IMGUI_API ImGuiWindow
{
    ...
    bool                    WasActive;
    bool                    WriteAccessed;                      // Set to true when any widget access the val window
    ...
    short                   BeginCount;                         // Number of Begin() during the val frame (generally 0 or 1, 1+ if appending via multiple Begin/End pairs)
