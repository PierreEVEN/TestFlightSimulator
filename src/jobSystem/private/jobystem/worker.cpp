...
{
    if (auto* worker = Worker::get()) { return worker->get_worker_id(); }
    return 255;
}
...
Worker* Worker::get_worker(size_t worker_id) { return workers + worker_id; }
...
{
    while (!job_pool.is_empty() || jobs > 0) { workers_complete_semaphore.acquire(); }
}

void Worker::destroy_workers()
{
    for (int i = 0; i < worker_count; ++i) { workers[i].run = false; }
    wake_up_worker_condition_variable.notify_all();
    for (int i = 0; i < worker_count; ++i) { workers_destroy_semaphore.acquire(); }
    LOG_INFO("no more job - destroyed workers");
    free(workers);
}

size_t Worker::get_worker_count() { return worker_count; }

void Worker::wake_up_worker() { wake_up_worker_condition_variable.notify_one(); }
...
Worker::Worker(const uint8_t worker_id)
    : id(worker_id), worker_thread([]() {
          ...
          Worker* worker;
          do {
              if (worker = get()) { worker->next_task(); }
              else
...
std::shared_ptr<IJobTask> Worker::steal_task()
{
    for (size_t i = 0; i < worker_count; ++i)
    {
        ...
        if (auto task = workers[i].current_task->steal_task()) { return task; }
    }
    return nullptr;
}
};
